...
pub(crate) struct ProteusSocket {
    ...
    /// Addresses whose last datagram needed **non-default addressing**, with what it needed and when (#234,
    /// #13).
    ///
    ...
    /// entry is only created by a datagram that **opened**, which takes a community secret this node holds.
    /// A stranger cannot flood the map to evict a real joiner, because a stranger's datagram never opens at
    /// all.
    ///
    /// **It stores the whole [`OpenedUnder`], not a flag** (#13). It began as "does this peer speak genesis",
    /// one bit for the one axis that existed; a peer can also be behind on the community secret, or — joining
    /// during a rollover — behind on both, and a bit cannot say which. Storing the answer the shaper already
    /// computed is also what keeps the two layers from re-deriving it differently.
    addressing: ReplyAddressing,
}

/// The peer-addressing map, shared between the datagram envelope that *learns* it and the frame layer that
/// must *act* on it (#234, #13).
///
...
pub(crate) type ReplyAddressing = Arc<Mutex<BoundedMap<SocketAddr, (OpenedUnder, Instant)>>>;

/// A fresh map, sized and expiring as [`ProteusSocket::addressing`] documents.
pub(crate) fn reply_addressing() -> ReplyAddressing {
    Arc::new(Mutex::new(BoundedMap::new(MAX_INBOUND_CONNECTIONS)))
}

/// How the next frame or datagram to `dst` must be addressed, or `None` for the ordinary path.
///
/// Free rather than a method, because the socket and the driver ask it of the same handle.
pub(crate) fn addressing_for(map: &ReplyAddressing, dst: SocketAddr) -> Option<OpenedUnder> {
    let map = map.lock().unwrap_or_else(PoisonError::into_inner);
    map.get(&dst).filter(|(_, at)| at.elapsed() < DIAL_TIMEOUT).map(|(under, _)| *under)
}
...
impl std::fmt::Debug for ProteusSocket {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let joining = self.addressing.lock().unwrap_or_else(PoisonError::into_inner).len();
        f.debug_struct("ProteusSocket").field("joining", &joining).finish_non_exhaustive()
    }
}

impl ProteusSocket {
    /// Remember how each of `peers` reached us, and drop rows past their window.
    ///
    ...
    fn note_addressing(&self, peers: &[(SocketAddr, OpenedUnder)]) {
        let now = Instant::now();
        let mut map = self.addressing.lock().unwrap_or_else(PoisonError::into_inner);
        map.retain(|_, (_, at): &(OpenedUnder, Instant)| now.duration_since(*at) < DIAL_TIMEOUT);
        for (addr, under) in peers {
            map.insert(*addr, (*under, now));
        }
    }
}
...
impl AsyncUdpSocket for ProteusSocket {
    ...
    fn try_send(&self, transmit: &Transmit<'_>) -> io::Result<()> {
        ...
        // mid-join and holds the window `{0, 0, 1}`; one that reached us under a retired community secret
        // does not hold this node's emitting one. Either way, sealing the reply the default way leaves it
        // unreadable and the handshake fails from the other side — the asymmetry #235 measured.
        let shaper = self.shaper.read().unwrap_or_else(PoisonError::into_inner);
        let sealed = match addressing_for(&self.addressing, transmit.destination) {
            Some(under) => shaper.seal_datagram_under(transmit.contents, &nonce, under),
            None => shaper.seal_datagram(transmit.contents, &nonce),
        };
        drop(shaper);
    ...
    fn poll_recv(
        ...
        loop {
            ...
            let mut joining: Vec<(SocketAddr, OpenedUnder)> = Vec::new();
            for i in 0..n {
                ...
                // Both axes, through one predicate the shaper owns — a caller that tested only the arm it
                // happened to know about would leave the other one silently unanswerable.
                if under.needs_addressing() {
                    joining.push((m.addr, under));
                }
                if kept != i {
