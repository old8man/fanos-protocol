...
//! ## The relation
//!
//! An output note is `cm = hash(value_node, owner)` ([`crate::ring_note`]'s leaf form), where `owner` is the
//! recipient's owner tag (`hash(nsk, nsk)` — the sender knows it from the recipient's address, but it stays hidden
//! on the ledger) and `value_node` encodes the amount. With `cm` and `Cv` **public**, the proof shows, in zero
//! knowledge over hidden `value_node`, `owner`:
//!
//! ```text
//! cm  = hash(value_node, owner)     (a public-output hash step — as the nullifier proof reveals nf)
//! value_node ↔ Cv                    (the value-tie: value_node encodes Cv's amount — crate::ring_value_tie)
//! value_node, owner short            (node-shortness, so the SIS hash relation is binding)
//! ```
//!
...
//! proof deliberately does **not** constrain how `owner` is derived: the sender does not know the recipient's
//! `nsk`, and ownership is established by the recipient's *own* spend proof ([`crate::ring_note`]) later; here
//! `owner` need only be a short node so the leaf's opening is well-defined.
//!
...
/// commitment: `cm = hash(value_node, owner)` for short `value_node`, `owner`, with `value_node` encoding `Cv`.
...
pub struct OutputProof {
    ...
    cm_step: HashStepProof,          // cm = hash(value_node, owner) — cm public
...
/// Prove a created output: the note commitment `cm = note_hp.hash(value_node, owner)` (the new tree leaf) is
/// well-formed, `value_node` and `owner` are short (`< 2^bits`; `bits = LOG_BASE` for real nodes), and
/// `value_node` encodes the amount committed by the public value commitment `cv`. `owner` is the recipient's owner
/// tag (hidden on the ledger). Returns the leaf `cm` (appended to the tree) and the proof.
...
pub fn prove_output(
    ...
    let cm = note_hp.hash(value_node, owner);
    ...
    // cm = hash(value_node, owner) — value_node is the left input, owner the right (matching crate::ring_note, so
    // the leaf equals the one the recipient's own note proof reconstructs at spend time).
    ...
    let owner_short = prove_node_short(params, owner, &owner_r, bits, &sub(seed, b"/oshort"))?;
...
pub fn verify_output(
    ...
    // cm = hash(value_node, owner); value_node ↔ Cv; both hidden nodes short; the public leaf is a valid hash node.
...
mod tests {
    ...
    /// A small node whose limbs have coefficients `< 2^4` — a stand-in owner tag for the fast (`bits = 4`) test.
    ...
    fn a_well_formed_output_proves_and_a_wrong_amount_is_rejected() {
        ...
        let owner = small_node(7); // an artificial (short) recipient owner tag

        let (cm, proof) = prove_output(&params, &note_hp, &cv, &rv, &vn, &owner, 4, b"seed").expect("output");
    ...
    fn a_leaf_from_a_different_note_is_rejected() {
        ...
        let other = note_hp.hash(&value_node(v), &small_node(9)); // a leaf of a different (owner) note
