...
| **Active probing & DPI blocking** (GFW fingerprints bridges and probes to confirm them) | the wire has a *static signature* and bridges are a *static list* | **PROTEUS** — polymorphic per-packet junk, beacon-rotating shape, **moving-target bridges** (derived from the beacon, no static list). Nothing static to enumerate. **Not yet true of the fingerprint:** shaping starts at the QUIC stream, so the connection still opens with a plaintext `fanos/1` ALPN and answers an unauthenticated prober — measured in `fanos-quic/tests/probe_resistance.rs`. `[T]` core / `[C]` bridges |
