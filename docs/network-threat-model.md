...
| A9 | **Reflection / third-party amplification** (a frame makes a node send traffic to a victim *outside* the network) | The class A7 does not reach: every A7 bound — canonical coordinates, the monotone re-flood guard, response ≤ request — is about traffic *inside* the overlay, and none of it applies when the destination is an arbitrary address a peer named. The answer is that **no address off the wire is ever dialed on a peer's say-so**: the hub's `peer_addrs` table is written only from `conn.remote_address()` (observed, never claimed), so a broker can only hand out an endpoint it personally saw; and the one handler that did act on a claimed address, `PunchTo`, now holds at most one outstanding dial per *coordinate* — a bound that falls out of the plane's point count rather than a constant — and writes the directory only after the dialed peer proves that coordinate | ✅ | **#78 ✅** — `fanos-quic::driver::accept_holepunch`; `tests/real_nat.rs::a_flood_of_punch_frames_cannot_aim_this_node_at_a_third_party` measures arrivals **at the victim** (an address on the modelled network owned by no node): 40 crafted punch frames from one admitted peer delivered **160** datagrams before the fix and **4** after — one dial's worth, whatever the flood. Falsified by deleting the claim |
...
| C9 | **Replay path-confirmation** (re-inject a captured cell to confirm a relay is on-path) | Bounded per-relay replay cache keyed on `sealed::replay_tag` (drops a recurring cell before decap); relay-key rotation (E4) is the second line | ✅ | `aphantos/tests/replay_attack.rs` (replay dropped, distinct cells forwarded) + **`sim/tests/replay.rs`** over the running mixnet: a genuine forwarded onion, captured and re-injected to the hop that already saw it, produces **no second delivery** (the replay cache drops it) while a distinct onion still routes; **E4 rotation ✅ (#61)** — `fanos-pqcrypto::OnionKeyRatchet` per-epoch forward-secure onion keys (bounded grace window) wired into `ThresholdRouter`, so a recorded cell is unpeelable once the relay ratchets past its epoch |
| C10 | **Predecessor attack** (identify the initiator by counting predecessors over repeated circuits) | A stable per-client **guard**, generalized to a slowly-rotating **guard set** (`fanos-nyx::GuardSet`): an ordered, **primary-first** set keeps exposure ≈`f` (the primary carries every circuit while up — *not* the `1−(1−f)^k` union bound a naive "any of k" set suffers), falls back to a **stable backup** under churn (availability without reopening the attack), and re-draws only on a coarse `epoch/rotation_period` **window** (slow rotation bounds lifetime exposure) | ✅ 🟡 | `nyx/tests/predecessor.rs` (guardless 1.000 → single-guard/guard-set ≈ f; the primary-first set matches single-guard exposure, **not** the union bound; survives primary churn) + `nyx::guard` unit tests (distinct/ordered set, window-stable slow rotation, primary-first failover). **`NyxNode` now enters through the guard SET** (`node::next_circuit`): the highest-priority *sealable* guard (present in the mix directory) carries each circuit, falling back to a stable backup when the primary is unsealable rather than to a guardless (predecessor-vulnerable) circuit — validated by `aphantos::node::tests::the_guard_set_falls_back_to_a_stable_backup_when_the_primary_is_unsealable`. Residual: the standalone engine has no epoch source, so the slow-rotation window is fixed (inert) there; the shipping proxy uses operator-configured forward circuits (the entry is the operator's choice on that path) |
