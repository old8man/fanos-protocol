...
### E1 — Full/threshold profile emits no cover traffic *(HIGH)*

`fanos-aphantos/src/threshold_router.rs` is the engine for `Profile::Full` (`profile.rs:28`). It has `with_mixing`/`mix_pending` (Poisson delay + reorder) but **no cover-cell emission** — no `cover_interval`, no `covering`, no generator (`mixing.rs` holds only the analytic formulas). The Lite engine `NyxNode` (`node.rs:164-197` `emit_cover`/`start_cover`) *does* have indistinguishable cover. On the threshold path a GPA sees real onions only, so link/volume/timing correlation is unmitigated and Full's "verifiable mixing / > Nym" claim (`profile.rs:15,38`) is not met. **Fix:** port `emit_cover` (constant-rate, same-size fail-to-peel cells) into `ThresholdRouter`, or route it through the same cover scheduler.

### E2 — Threshold mix delays are a public, predictable function *(HIGH)*

`threshold_router.rs:122-136` — `sample_delay` seeds the exponential from `self.coord.coords()` (the node's **public** projective coordinate) concatenated with `id = mix_seq` (a 1,2,3… counter). No secret is mixed in. An adversary who knows a node's coordinate — which is public — can recompute the entire delay sequence `D(coord,1), D(coord,2), …` a priori and re-link a hop's in/out flows, defeating the exact timing correlation Poisson mixing exists to destroy. `NyxNode::sample_delay` (`node.rs:128-144`) does this correctly, keyed by the secret `self.seed`; the threshold router is a one-line-fixable regression. **Fix:** derive the delay from a secret (`hash_labeled("…mix", kem_secret ‖ id)`), as `NyxNode` does.
...
### E4 — Forward secrecy is sender-side only; no relay-key rotation *(MEDIUM)*

The KEM encapsulates to relays' **long-term** hybrid keys (static X25519 + static ML-KEM). FS holds against sender build-randomness compromise but **not** against relay-key compromise: a GPA that records onion `kem_ct` bytes and later compromises a relay's long-term secret decrypts all past hops through it — the standard mixnet FS threat. `profile.rs:13` advertises "unpredictable epochs," but no per-epoch ephemeral relay-key rotation is wired into the sealing path. **Fix:** rotate relay KEM keys per epoch (publish short-lived encapsulation keys), or state plainly that FS is sender-side only.

### E5 — Rendezvous "VRF beacon" is a predictable hash *(MEDIUM)*

`fanos-calypso/src/rendezvous.rs` — `rendezvous_line = MapToLine(H("FANOS-v1/calypso" ‖ pubkey ‖ epoch))`, a plain deterministic hash; the spec's `VRF_beacon` is not wired in. All future meeting lines are computable arbitrarily far ahead from public data, so an adversary can pre-position on, block, or surveil a service's rendezvous line — defeating the claimed unpredictability. This is a second concrete site (with A7) where the built-but-unused `fanos-vrf` belongs. **Fix:** wire the VRF beacon into the line derivation.

### E6 — Cover traffic is additive, not constant-rate *(MEDIUM)*

Even on the Lite path, `node.rs:164-197` `emit_cover` sends cover *on top of* real forwards, so total emission = real + cover and send volume still rises with real load — undercutting the "no volume fingerprint (a theorem)" claim (`mixing.rs:6-8`). It is also single-hop drop-cover, not Loopix loop-cover (link cover, not path cover). **Fix:** constant-rate send-or-cover scheduling where real traffic consumes cover slots, matched per line.
...
**Verdict.** The residuals are largely documented in-code and none is a fabrication, but the marketing verbs — "verifiable mixing," "forward secrecy," "unpredictable epochs," "no volume fingerprint" — run ahead of what the shipping engines enforce. Fixing E1 and E2 (both small; E2 is a one-line parity fix with `NyxNode`) closes the largest and most surprising gap; guard E3 regardless of profile.
...
5. **Restore the Full-profile anonymity floor (E1, E2):** port constant-rate cover into `ThresholdRouter`, and key its mix delays off a secret (E2 is a one-line parity fix with `NyxNode`). These close the largest and most surprising anonymity gap.
