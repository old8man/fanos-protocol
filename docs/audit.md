...
> | **AT-H2** reputation decay + miss path unwired | **PARTLY RESOLVED — my first verdict here was wrong** | The refund half IS wired: `hybrid.rs:498` `finalize_lapsed_deals` runs inside `apply_block`, auto-completes any deal past `open_height + duration·AUDIT_PERIOD`, refunds the unproven remainder to the consumer, and prunes terminal deals — its own comment cites AT-H2. I first read this as STILL OPEN by grepping for `settle_epoch(.., false, ..)` / `Settlement::Miss` / `Reputation::observe` and finding none; the remedy simply took a different shape, and **a grep for symbol names finds absent names, not absent properties**. Two genuine residuals remain: (a) no *early* termination — a provider that stops proving on day one still locks the consumer's escrow for the full term, because misses are never settled per epoch even though `settle_epoch` supports it; (b) `Reputation` does not exist anywhere in `fanos-thesauros`, so a bad provider carries no penalty beyond that one deal |
...
> genuinely open when swept — AT-H3 (closed the same day) and AT-H2 (whose verdict was itself wrong, see its row) — and one is partial (O-H2).** The two open ones are tracked as tasks. Nothing was
...
- **MEDIUM:** ~~AT-M1 no key zeroization anywhere in ANGELOS (no `zeroize` dep, no `Drop`/`Zeroize`)~~ — **CLOSED, and the struck text was false on BOTH clauses when re-measured**: `zeroize` is a dependency (`fanos-angelos/Cargo.toml:21`) and four key-holding types carry `impl Drop` citing AT-M1 by name (`SendChain`, `RecvChain`, `MediaSession`, `DoubleRatchet`). The fifth, `Attachment`, genuinely lacked it and lacked a redacted `Debug` besides — its derived one printed all 32 key bytes; both closed in #342. Scan boundary, so the count is not mistaken for proof: the enumeration matched a `[u8; 32]` field named key/secret/root, or `SecretKey`/`HybridKemSecret`. Five is a LOWER BOUND; AT-M2 the group session drops all out-of-order messages while channel text rides the reordering Full mixnet (permanent loss); AT-M3 per-chunk PoR soundness is capped at the leaf count (`0.9^64 ≈ 9.7` bits, not the advertised λ=20/30/40 — the market treats one chunk pass as the epoch's proof); AT-M4 `close_deal` accepts any consumer signature unbound to `(deal_id, close, height)` → replay a historical `SignedTransfer` to force-close deals early (griefing); AT-M5 session/ratchet randomness is caller-seeded (`SeedRng` satisfies the `CryptoRng` bound) → a weak/reused seed silently breaks FS+PCS.
- **LOW:** media cleartext `epoch‖seq` flow-fingerprint; ~~`unwrap_or_default` masks AEAD-seal failure~~ **CLOSED (e3dbe53)** — four sites, and the loudest was not in ANGELOS at all: `thesauros::seal_object` committed the EMPTY ciphertext to the manifest under a valid `chunk_cid`, so a failed write stored, addressed and verified as intact. Zero `unwrap_or_default` on any seal path remain, measured; unbounded Merkle-path length; pre-auth skip-key derivation (bounded ~µs); `audit_beacon` inits to zero (latent).
...
OBOLOS O-M1/M2/M3/M4; ANGELOS AT-M2 (group out-of-order drop), AT-M3 (per-chunk PoR < λ), AT-M5 (caller-seeded ratchet randomness), AT-H3 (media replay window); TAXIS T-H3 (keyper censorship), T-H5 (slashing/rewards emitted but never applied to state), T-M1-M4 (cross-cell); C7 (telemetry-DP built-but-unwired — zero `.privatize` callers), holonomy verification (absent from the threshold/rendezvous peel path). None regressed; each remains as characterized — **except three now closed**, and all three had gone stale in this list. **AT-M1** (ANGELOS key zeroization) was the third, and it is the one worth reading twice: the entry did not merely age, it was *false in the direction that hides shipped work* — four `impl Drop`s and a `zeroize` dependency existed while the register said none did. A register that under-claims is as much a defect as one that over-claims, because the next reader re-does the work; see #342 for the residual it did correctly imply.
