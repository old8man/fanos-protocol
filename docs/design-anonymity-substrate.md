...
A captured *middle* hop learns its predecessor and successor — relays, not the client or the destination. The
deanonymization event is **first-and-last correlation**. And the relevant combinatorics are not probabilistic:
**two points lie on exactly one line**, so each corrupt *pair* captures exactly one hop.
...
The capture column is hypergeometric — the corrupt count is exactly `f`, not a binomial draw — and it reproduces
the exact enumeration at `q = 2` (`1/7 = 0.1429`), which is what confirms the model. The worst-case column is
the pigeonhole bound `min(C(f,2), n)/n`: each corrupt *pair* determines one line, capping what deliberate
placement could reach.

At `q = 2` the tolerance is `f = 2 = t`: one pair, one captured line — and one line cannot be both the first and
the last hop, so **end-to-end deanonymization is impossible at the default plane**. Exact, not merely small.

That is an accident of `f = t`, and it does not survive. Above Fano the pair count outruns the line count and
the tolerated corruption alone owns every hop: **at `q ≥ 7` the MIX lane provides no anonymity under the
platform's own fault assumption**, and raising `q` makes matters worse rather than better.

### The threshold that should be there

Under the platform's tolerance the corrupt *density* is `f/n = ⌊(n−1)/3⌋/n → 1/3`, so a line of `m` points
carries about `m/3` corrupt in expectation. The threshold preserving Fano's margin at any `m` is

```text
t = ⌈2m/3⌉ = ⌈2(q+1)/3⌉        which at q = 2 evaluates to exactly 2 — the shipped constant
```

The value is right; it was never generalized. With it, hop capture **falls** as the plane grows, which is what
§4 promises and what the constant prevents:

| `q` | `t` now | `t` fixed | P(hop) now | P(hop) fixed |
|---|---|---|---|---|
| 2 | 2 | **2** | 0.143 | 0.143 |
| 4 | 2 | 4 | 0.450 | 0.0114 |
| 7 | 2 | 6 | 0.795 | 0.0091 |
...
**Liveness is the cost to check before this lands.** A higher `t` needs more of a line reachable to peel a hop —
two thirds, the same ratio Fano already runs at. That is the trade BFT makes everywhere else; it should be
verified against the mixnet's liveness tests rather than assumed. Tracked in `docs/audit.md` E7.

### Two wrong turns, recorded because both are reusable

**A bound quoted as a property.** §4's Chernoff bound evaluated at `q = 2` gives `0.425`, which looked like a
weakness. But a binomial models hops as *independent draws*, and on one finite plane the corrupt set is fixed
and hops share points. Enumeration gives `P(first ∧ last) = 0` at two corrupt, not the `3.9 %` the binomial
predicted.

**A result generalized without re-derivation.** Having found the Fano guarantee exact, the tempting conclusion
was "the design is better than its justification" — true at `q = 2` and false everywhere else, because it rested
on `f = t`.

*Provenance:* the enumeration was run on a hand-written line set and again on lines generated by the
repository's own `fanos-geometry::fano` GF(2) construction. Different labellings, identical numbers.
