...
**Which types are critical, and why exactly one group is.** The rule above was stated here, in `fanos-wire`'s module header and in `activation.rs`, and implemented in none of them — `WireError::UnknownCriticalFrame` existed with no site that could construct it, so *every* unknown frame was skipped. Criticality is now computed from the group (`FrameType::group_is_critical`), and it is derived rather than listed: **skipping is right when ignorance costs availability, and wrong when it costs agreement.** An unread `LOOKUP` is a missed read, an unread `TESSERA` a dropped circuit, an unread `APP` an application's own problem. `0x1*` Membership is the exception: `BEACON` rounds fix the epoch every coordinate, meeting point and roster derives from, and `BEACON_RESHARE_*` retires the key material underneath them. A node that skips those does not fail — it keeps serving on a retired epoch, indistinguishable from a healthy one, which is precisely the silent divergence §3 of `docs/design-upgrade.md` exists to prevent. Aborting does not make an old node obey a frame it cannot parse; it makes it stop participating rather than participate wrongly, which is the fail-closed half capability negotiation cannot supply (capabilities govern what a well-behaved peer *sends*; this governs what an old node does when one arrives anyway).
...
**Status (#99):** the per-intro **threshold-decryption is live-wired** — the production `ThresholdService` engine (`fanos-node`) runs the `t`-of-`(q+1)` PartialDec gather over the overlay (`RdvIntro`/`SvcShareReq`/`SvcPartial` frames) and surfaces the recovered request; verified over the sim (any threshold subset serves, `< t` is 0-knowledge, concurrent intros multiplex). A client **discovers** the line through a wire-serialized `ServiceLine` roster (`fanos_calypso::hosting` — the members' public keys in seal order, their coordinates, the threshold) and seals its intro to it: the full discover → seal → serve loop is proven end to end. Identity custody (`deal_service_key`) and the sharing/opening primitives are built and unit-tested. Remaining: composition with the anonymous transport (§12.4) and LRC-replicated service state.
...
- **PQ-VRF, PQ beacon, PQ verifiable shuffle are [P].** Active crypto directions; we use classical variants as an interim, honestly marked. **The three are not peers in blast radius.** A classical VRF or shuffle is *per-node* and *forward-looking* — a future adversary forges future proofs. The beacon is **global and retroactive**: its group commitment is a Feldman commitment `x·G` over ristretto255, published in every node's configuration and, through `genesis_seed = H(label ‖ commitment)`, serving as the network's public name — so it can never be withdrawn. One discrete-log solve on that public value recomputes `seed(epoch) = H(x·M(epoch))` for **every past epoch**, and meeting points, ingress-line rosters, role assignment and coordinates are all pure functions of `(community, epoch, beacon)`. An adversary who archives one configuration file today can, post-quantum, replay the network's entire recorded history. See `docs/network-threat-model.md` E6.
